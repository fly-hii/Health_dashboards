...
export default function Appointments() {
  ...
  // Filter appointments by selected date (local time YYYY-MM-DD)
  const filteredAppointments = appointments.filter(appt => {
    if (!selectedDate) return true;
    const apptDate = new Date(appt.date_time || appt.dateTime);
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative flex items-center">
            ...
            <input
              ...
              className="pl-9 pr-8 py-2 bg-white border border-slate-200 rounded-xl text-xs text-slate-700 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary"
            />
            {selectedDate && (
              <button
                type="button"
                  ...
                  <tr key={appt.id || appt._id} className="border-b border-slate-50 hover:bg-slate-50/50">
                    ...
                    <td className="py-4">
                      <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold border ${
                        appt.status === 'Confirmed' || appt.status === 'Completed' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' :
                        appt.status === 'Pending' ? 'bg-amber-50 text-amber-700 border-amber-200' :
                        appt.status === 'In-Progress' ? 'bg-blue-50 text-blue-700 border-blue-200' :
                        appt.status === 'Cancelled' || appt.status === 'No-Show' ? 'bg-rose-50 text-rose-700 border-rose-200' :
