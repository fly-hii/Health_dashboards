'use strict';
/**
 * forgotPasswordController.js – Nurse Backend
 * Endpoints:
...
// In-memory OTP store for forgot-password. For production, use Redis.
...
const sendForgotPasswordOtp = async (req, res) => {
  try {
    ...
    if (!['NURSE'].includes(user.role)) {
      return res.status(403).json({ success: false, message: 'This email is not registered as a Nurse in this portal.' });
    }
    ...
    try {
      await sendOtpEmail(user.email, user.name || 'Nurse', otp, 'Nurse Portal');
    } catch (emailErr) {
...
const resetForgotPassword = async (req, res) => {
  try {
    ...
    console.error('Nurse reset password error:', error);
    res.status(500).json({ success: false, message: 'Server error' });
  }
};
